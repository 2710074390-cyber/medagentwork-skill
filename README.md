...
| **无自有素材 / 金标准** | 本 skill 不含任何真题、教材、图谱；金标准只能走降级模式，**该批次无兜底** |
| **想开箱即用出图** | 生图依赖外部图片 Agent，未随包分发 |
...
图片生成由外部图片 Agent 执行（原「豆包」链路未随包分发）。你拿到的是**占位符 + 配图清单**，图需自行解决。没有该能力时请直接跳过插图阶段。

### 5. 不含任何版权数据

无真题、无教材、无图谱。`kaoyan_picker`（金标准配额）与 `compose_atlas`（图谱复用）**只有机制没有数据**。
...
| 项 | 说明 |
|---|---|
| **巨型文件未拆分** | `scripts/render_review.py`（1520 行）、`scripts/gate_check.py`（1255 行）尚未按职责拆分（评审 §七.8）。前置条件：先补渲染快照回归基线 |
| **提示词语境残留** | 五个角色提示词源自原项目；v2.1 已清洗路径，但正文语境仍带原项目痕迹（如多 Agent 工作区假设） |
...
- 核心管线需 **Python 3.10+**；插图四件套另需 `Pillow` + `PyYAML`（见 `requirements-optional.txt`）
